...
public class ConsoleMenu {
    ...
    public void run() {
        while (true) {
            ...
            System.out.println("1) Zobraz zákazníka a účty");
            System.out.println("2) Přidat příjemce (Beneficiary)");
            System.out.println("3) Odeslat platbu příjemci ze seznamu");
            System.out.println("4) Odeslat platbu na IBAN");
            System.out.println("5) Autorizovat platbu (UC 05)");
            System.out.println("6) Fraud alerts: vypsat / approve / decline / request confirm (UC 11/12)");
            System.out.println("7) Storno platby (UC 19)");
            System.out.println("8) Výpis převodů podle účtu");
    ...
    private void showCustomerAndAccounts() {
        ...
        System.out.println("\nZákazník: " + cust.name() + " (id=" + cust.id() + ")");
        System.out.println("Email: " + cust.email());
        System.out.println("Adresa: " + cust.address().street() + ", " + cust.address().city());
    ...
    private void addBeneficiary() {
        var cust = infra.customers.byId(customerId).orElseThrow();
        System.out.print("Jméno příjemce: ");
        String name = in.nextLine().trim();
        System.out.print("IBAN (např. CZ0201000000000012345678): ");
        String iban = in.nextLine().trim();
        System.out.print("Důvěryhodný? (y/N): ");
        ...
        System.out.println("[OK] Přidán příjemce id=" + bid);
    }

    private void submitPaymentByBeneficiary() {
        var accounts = infra.accounts.byCustomerId(customerId);
        if (accounts.isEmpty()) { System.out.println("Žádné účty"); return; }
        int accId = askInt("Zdrojový účet id", accounts.get(0).id());
        int benId = askInt("ID příjemce", -1);
        double amount = askDouble("Částka CZK", 1000);
        int tid = services.transferService.submitPaymentByBeneficiary(customerId, accId, benId, amount, "");
        System.out.println("[OK] Vytvořen transfer id=" + tid);
    }
    ...
    private void submitPaymentToIban() {
        ...
        if (accounts.isEmpty()) { System.out.println("Žádné účty"); return; }
        int accId = askInt("Zdrojový účet id", accounts.get(0).id());
        System.out.print("Cílový IBAN: ");
        String iban = in.nextLine().trim();
        double amount = askDouble("Částka CZK", 6000);
    ...
    private void authorizePayment() {
        ...
        System.out.println("[Výsledek] Transfer " + tid + " má stav " + t.status());
        var acc = infra.accounts.byId(t.sourceAccountId()).orElseThrow();
        System.out.println("Účet id=" + acc.id() + " balance=" + acc.balance());
    }

    private void fraudMenu() {
        var list = infra.alerts.all();
        if (list.isEmpty()) { System.out.println("Žádné alerty"); return; }
        ...
        switch (act.toLowerCase()) {
            ...
            default -> System.out.println("Neznámá akce");
        }
    }
    ...
    private void listTransfersByAccount() {
        var accounts = infra.accounts.byCustomerId(customerId);
        if (accounts.isEmpty()) { System.out.println("Žádné účty"); return; }
        int accId = askInt("Účet id", accounts.get(0).id());
        var list = infra.transfers.bySourceAccount(accId);
        if (list.isEmpty()) { System.out.println("Žádné převody"); return; }
        for (Transfer t : list) {
