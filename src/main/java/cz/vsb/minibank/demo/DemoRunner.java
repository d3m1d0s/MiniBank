// =========================
// Iteration 5 — Demo & Post-conditions Checker
// =========================
// Package: cz.vsb.minibank.demo
// Purpose: Runs an automated scenario through UC 04 → 05 → 11 (+ cancel UC 19)
// and prints verifiable post-conditions. Uses the domain + application layer.
// Data file: data/demo.json (kept separate so it does not affect manual tests in data/data.json)
...
public class DemoRunner {
    public static void main(String[] args) {
        String dataPath = "data/demo.json"; // separate file
        ...
        try {
            ...
            // Submit a payment to an IBAN above dailyLimit (6000 CZK) → WAITING_AUTH, then authorize → SENT.
            // ===============
            ...
            assertState(tr1.status() == TransferStatus.WAITING_AUTH, "T1 must be WAITING_AUTH after creation");
            services.transferService.authorizePayment(t1, "0000");
            tr1 = infra.transfers.byId(t1).orElseThrow();
            assertState(tr1.status() == TransferStatus.SENT, "T1 must be SENT after authorization");
            ...
            assertState(eq(afterT1, expectedBal1), "Balance after T1 must be reduced by amount + fee");
            System.out.println("[OK] UC04→UC05: T1 SENT, balance=" + afterT1 + " (fee=" + expectedFee1 + ") \n");

            // ===============
            // 2) UC 04 → 11 → 05 (FRAUD ALERT + APPROVE + AUTH)
            // New untrusted beneficiary + amount 12,000 CZK → FraudAlert{NEW} + WAITING_AUTH.
            ...
            assertState(tr2.status() == TransferStatus.WAITING_AUTH, "T2 must be WAITING_AUTH after creation");
            var alert2 = infra.alerts.byTransferId(t2).orElseThrow();
            assertState(alert2.state() == FraudAlertState.NEW, "Alert for T2 must be NEW");

            services.fraudService.approve(t2); // alert → OK, transfer remains WAITING_AUTH (requires customer authorization)
            alert2 = infra.alerts.byTransferId(t2).orElseThrow();
            assertState(alert2.state() == FraudAlertState.OK, "Alert for T2 must be OK after approve");
            tr2 = infra.transfers.byId(t2).orElseThrow();
            assertState(tr2.status() == TransferStatus.WAITING_AUTH, "T2 still has WAITING_AUTH (customer authorization required)");

            services.transferService.authorizePayment(t2, "123456");
            tr2 = infra.transfers.byId(t2).orElseThrow();
            assertState(tr2.status() == TransferStatus.SENT, "T2 must be SENT after authorization");
            ...
            assertState(eq(afterT2, expectedBal2), "Balance after T2 must reflect the second outgoing payment");
            System.out.println("[OK] UC04→UC11→UC05: T2 SENT, balance=" + afterT2 + " (fee=" + expectedFee2 + ") \n");

            // ===============
            // 3) UC 19 (Cancel) — create a pending payment and cancel it; the balance stays unchanged.
            // ===============
            ...
            assertState(tr3.status() == TransferStatus.WAITING_AUTH, "T3 must be WAITING_AUTH after creation");
            services.transferService.cancelPayment(t3);
            tr3 = infra.transfers.byId(t3).orElseThrow();
            assertState(tr3.status() == TransferStatus.DECLINED, "T3 must be DECLINED after cancel");

            acc = infra.accounts.byId(accountId).orElseThrow();
            Money afterT3 = acc.balance();
            assertState(eq(afterT3, expectedBal2), "Balance must remain unchanged after canceling T3");
...
/*
RUN:
  1) Add the Jackson dependency (if not already present) and build the project.
  2) Run `cz.vsb.minibank.demo.DemoRunner`.
EXPECT:
  - T1: WAITING_AUTH → (Authorize 0000) → SENT, balance reduced by 6000 + 30.
  - T2: FraudAlert{NEW} created, approve → alert OK, transfer WAITING_AUTH → Authorize 123456 → SENT.
  - T3: WAITING_AUTH → Cancel → DECLINED, balance unchanged.
  - The file `data/demo.json` will contain the new customer/account/beneficiary/transfers/alerts with states as above.
*/
