...
import { filterProblem } from '@shared/logic/alertFilters';
/*
 ...
 */
import { formatFeeLine } from '@shared/text/money';
/*
 ...
 */
import { goTo, navigateToView, replaceRoute, routeFor } from '@shared/nav/route';
import { describeApiError, describeApiFailure } from '@shared/wire/apiErrors';
import type { ApiFailure } from '@shared/wire/apiErrors';
...
import {
    ...
    nextPage,
    showingLine,
} from '@shared/logic/paging';
import { SESSION_IDLE_NOTE, type NavRole } from '@shared/nav/navigation';
...
function historyCells(h: HistoryItem, alertedIban: string | null): HistoryRowCells<ReactNode> {
    ...
    // The end this payment came to. A payment settles or it is stopped, so the second line of the
    // cell holds whichever of the two this row has and the word in front of it says which. The
    // settlement is preferred where a row somehow carries both, which the domain and the schema
    // each forbid and only a hand-written row could produce.
    ...
    return {
        ...
        createdAt: (
            <>
                ...
                {endedLine && (
                    // The class names the second line of this cell, not the fact printed in it,
